...
export async function POST(request) {
  try {
    ...
    if (process.env.CLERK_WEBHOOK_SECRET) {
      ...
      console.warn('⚠️ Webhook running without signature verification (development mode)');
    }
    ...
    switch (event.type) {
      ...
      case 'user.updated':
        ...
        console.log(`[CLERK-WEBHOOK] ${event.type} - will be synced on next user access`);
        break;
...
async function handleUserDeletion(clerkUserId) {
  console.log(`[CLERK-WEBHOOK] Starting cleanup for deleted user: ${clerkUserId}`);
  
  try {
    // Find user in our database
    const user = await User.findOne({ clerkId: clerkUserId });
    if (!user) {
      console.log(`[CLERK-WEBHOOK] User ${clerkUserId} not found in database - nothing to clean`);
      return;
    }

    // 1. Delete all user's files
    const deletedFiles = await File.deleteMany({ ownerId: clerkUserId });
    console.log(`[CLERK-WEBHOOK] Deleted ${deletedFiles.deletedCount} files for user ${clerkUserId}`);

    // 2. Delete all user's bots
    const deletedBots = await Bot.deleteMany({ ownerId: clerkUserId });
    console.log(`[CLERK-WEBHOOK] Deleted ${deletedBots.deletedCount} bots for user ${clerkUserId}`);
    ...
    console.log(`[CLERK-WEBHOOK] ✅ Cleanup completed for user: ${clerkUserId}`);

  } catch (error) {
    console.error(`[CLERK-WEBHOOK] ❌ Cleanup failed for user ${clerkUserId}:`, error);
    throw error;
  }
}