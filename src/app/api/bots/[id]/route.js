...
export async function GET(request, { params }) {
	try {
		...
		// Step 6: Format bot data with computed fields
		const botData = {
			id: bot._id.toString(),
			name: bot.name,
	...
	try {
		...
		if (updateData.customization !== undefined) {
			...
			if (
				...
				!/^#[0-9A-Fa-f]{6}$/.test(customization.bubbleColor)
			) {
...
export async function DELETE(request, { params }) {
	try {
		...
		// Step 6: Get deletion summary of files before deletion
		...
		try {
			...
			vectorCollectionDeleted = false;
			console.warn(
				`Failed to delete vector collection for bot ${botId}:`,
				vectorError.message
			);
		...
		try {
			...
			await Promise.all([Conversation.deleteMany({ botId: bot._id })]);
			console.log(`Deleted conversations for bot ${botId}`);
		} catch (modelError) {
			console.warn(
				'Conversation/Message models not available, skipping cleanup:',
				modelError.message
			);
		}
		...
		try {
			...
			user.usage.storage = Math.max(
				0,
				(user.usage.storage || totalSize) - totalSize
			);
			await user.save();
			console.log('User usage updated after bot deletion.');
		} catch (updateError) {
			console.warn(
				'Failed to update user statistics after bot deletion:',
				updateError.message
			);
		}
		...
		const deletionSummary = {
			...
			vectorCollectionDeleted, // Assume success unless error occurred
			deletedAt: new Date(),
		};

		return apiSuccess(
			deletionSummary,
			'Bot and all associated data deleted successfully'
		);
	} catch (error) {
