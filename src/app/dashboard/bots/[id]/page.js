...
export default function BotDetail({ params }) {
	...
	const handleConversationDetailRefresh = async () => {
		...
		try {
			const detail = await getConversationDetail(selectedConversation.sessionId);
			setConversationDetail(detail);
		} catch (error) {
			setConversationDetailError(error.message);
		} finally {
								...
								<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
									...
									<div>
										<input
											...
											onChange={(e) => filterByDateRange(e.target.value, filters.dateTo)}
											className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-orange-500"
										/>
									</div>
								</div>
							...
							<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[800px]">
								...
								<div className="lg:col-span-2">
									<ConversationDetail
										...
										botInfo={conversationDetail?.botInfo || { name: bot?.name, id: bot?.id }}
										onClose={() => {
											setSelectedConversation(null);
											setConversationDetail(null);
										}}
