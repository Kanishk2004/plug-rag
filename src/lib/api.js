...
export const apiUtils = {
  ...
  formatError(error) {
    if (error instanceof APIError) {
      switch (error.status) {
        ...
        case 429:
          return error.message || 'You have reached your plan limits. Please upgrade or try again later.';
        case 500:
  ...
  validateFile(file, options = {}) {
    ...
    if (file.size > maxSize) {
      return {
        isValid: false,
        error: `File size must be less than ${this.formatFileSize(maxSize)}. Current size: ${this.formatFileSize(file.size)}`,
      };
    }
