...
import { storeDocuments, getCollectionInfo, listCollections } from '../integrations/qdrant.js';
...
ASSISTANT RESPONSE:`);
    ...
    async getCollectionStatus(botId) {
        try {
            ...
            console.error(`💥 [RAG] Error checking collection status:`, error.message);
            return {
                exists: false,
                pointsCount: 0,
                vectorsCount: 0,
     ...
     */
    async storeDocuments(botId, apiKey, documents, metadata = {}) {
        try {
            const collectionName = botId.toString();
            console.log(`📥 [RAG] Starting document storage for collection: ${collectionName}`);
            console.log(`📄 [RAG] Documents to store: ${documents.length}`);
            ...
            console.log(`✅ [RAG] Documents stored successfully:`, result);

            // Verify storage
            const status = await this.getCollectionStatus(botId);
            console.log(`🔍 [RAG] Post-storage collection status:`, status);
    ...
    async retrieveDocuments(botId, apiKey, query, topK = 4) {
        try {
            const collectionName = botId.toString();
            console.log(`🔍 [RAG] Starting document retrieval for: ${collectionName}`);
            ...
            console.log(`✅ [RAG] Collection found with ${status.pointsCount} points`);
    ...
    async generateResponse(botId, apiKey, query, conversationHistory = []) {
        try {
            ...
            if (documents.length === 0) {
                console.log(`⚠️ [RAG] No relevant documents found - returning fallback response`);
                return this.generateFallbackResponse();
            }

            console.log(`📚 [RAG] Found ${documents.length} relevant documents, generating response...`);
            ...
            const sources = documents.map((doc, index) => {
                ...
                return {
                    fileName: metadata.fileName || metadata.source || metadata.filename || 'Unknown file',
    ...
    formatDocumentsAsContext(documents) {
        ...
        return documents
            .map((doc, index) => {
                const metadata = doc.metadata || {};
                const fileName = metadata.fileName || metadata.source || metadata.filename || 'Unknown file';
                const pageNumber = metadata.pageNumber || metadata.page || metadata.chunk;
                ...
                try {
                    ...
                    if (sampleResult.points?.length > 0) {
                        debugInfo.sampleDocuments = sampleResult.points.map(point => ({
                            ...
                            contentPreview: point.payload?.page_content?.substring(0, 100) + '...' || 'No content',
                            // Show original file references for easier debugging
                            originalFileId: point.payload?.metadata?.originalFileId || 'Unknown',
