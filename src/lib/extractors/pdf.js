...
export async function extractFromPDF(buffer, options = {}) {
	...
	try {
		...
		// Use simple buffer-based extraction for now
		const result = await extractWithSimpleMethod(buffer, options);
...
/**
 * Simple PDF text extraction using buffer analysis  
 */
async function extractWithSimpleMethod(buffer, options) {
	const { preserveFormatting = false } = options;
	
	// Convert buffer to string and extract readable text
	let text = buffer.toString('latin1');
	
	// Extract text content between stream objects
	const textMatches = text.match(/stream\s*([\s\S]*?)\s*endstream/g) || [];
	let extractedText = '';
	
	textMatches.forEach(match => {
		// Remove stream markers
		let content = match.replace(/^stream\s*|\s*endstream$/g, '');
		
		// Try to decode text content (basic approach)
		// Look for readable text patterns
		const readableText = content.match(/[a-zA-Z0-9\s.,!?;:'"()\-\[\]{}+=*&%$#@<>\/\\|`~_^]{10,}/g) || [];
		
		readableText.forEach(textChunk => {
			// Clean up the text
	...
	// If no text found through stream extraction, try simple text extraction
	if (extractedText.length < 100) {
		// Look for any readable text in the entire buffer
	...
	if (!preserveFormatting && extractedText) {
		extractedText = cleanPDFText(extractedText);
	}
	
	// If still no meaningful text, return a minimal result
	if (!extractedText || extractedText.length < 50) {
		extractedText = 'PDF file processed but text content could not be extracted. This may be an image-based PDF or encrypted document.';
	}
	...
	const result = {
		...
		metadata: {
			...
			extractionMethod: 'simple_buffer',
		},
		wordCount: countWords(extractedText),
...
function addOverlapToChunks(chunks, overlapTokens) {
	...
	for (let i = 1; i < chunks.length; i++) {
		...
		if (overlapWordCount > 0) {
			const overlapText = previousWords.slice(-overlapWordCount).join(' ');
