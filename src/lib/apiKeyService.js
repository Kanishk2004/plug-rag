...
export class ApiKeyService {
    ...
    try {
      ...
      if (options.models) {
        if (options.models.chat) {
          updateData['apiConfiguration.openaiConfig.models.chat'] = options.models.chat;
        }
        if (options.models.embeddings) {
          updateData['apiConfiguration.openaiConfig.models.embeddings'] = options.models.embeddings;
        }
      }
      
      // Set fallback preference if provided
      if (options.fallbackToGlobal !== undefined) {
        updateData['apiConfiguration.fallbackToGlobal'] = options.fallbackToGlobal;
      }
      
      // Set cost tracking options if provided
      if (options.costTracking) {
        if (options.costTracking.enabled !== undefined) {
          updateData['apiConfiguration.openaiConfig.costTracking.enabled'] = options.costTracking.enabled;
        }
        if (options.costTracking.monthlyLimit !== undefined) {
          updateData['apiConfiguration.openaiConfig.costTracking.monthlyLimit'] = options.costTracking.monthlyLimit;
        }
      }
  ...
  async getApiKey(botId, userId) {
    try {
      ...
      console.log(`[ApiKeyService] Getting API key for bot ${botId}, user ${userId}`);
      
      // Query bot with API configuration (including encrypted key)
      const bot = await Bot.findOne(
        { _id: botId, ownerId: userId }
      ).select('+apiConfiguration.openaiConfig.apiKeyEncrypted');
      
      if (!bot) {
        console.log(`[ApiKeyService] Bot not found or unauthorized for bot ${botId}, user ${userId}`);
        throw new Error('Bot not found or unauthorized');
      }
      ...
      console.log(`[ApiKeyService] Bot found. Has custom key: ${!!apiConfig?.apiKeyEncrypted}, Fallback enabled: ${fallbackEnabled}`);
      ...
      if (!apiConfig?.apiKeyEncrypted) {
        ...
        console.log(`[ApiKeyService] No custom key and no global fallback available`);
		...
		console.log(`[ApiKeyService] Custom API key decrypted successfully. Length: ${decryptedKey ? decryptedKey.length : 'undefined'}, starts with sk-: ${decryptedKey ? decryptedKey.startsWith('sk-') : 'undefined'}`);
      ...
      console.error(`[ApiKeyService] Error retrieving API key for bot ${botId}:`, error);
      throw error;
    }
  }
  ...
  async validateStoredKey(botId, userId) {
    try {
      ...
      await Bot.findOneAndUpdate(
        { _id: botId, ownerId: userId },
        {
          'apiConfiguration.openaiConfig.keyStatus': validation.isValid ? 'valid' : 'invalid',
          'apiConfiguration.openaiConfig.lastValidated': new Date()
        }
      );
  ...
  async trackUsage(botId, userId, usage) {
    try {
      ...
      if (usage.totalTokens) {
        updateData.$inc['apiConfiguration.openaiConfig.usage.totalTokens'] = usage.totalTokens;
      }
      if (usage.chatTokens) {
        updateData.$inc['apiConfiguration.openaiConfig.usage.chatTokens'] = usage.chatTokens;
      }
      if (usage.embedTokens) {
        updateData.$inc['apiConfiguration.openaiConfig.usage.embedTokens'] = usage.embedTokens;
      }
