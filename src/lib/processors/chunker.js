...
export function validateChunkQuality(documents) {
	...
	if (smallChunks.length > documents.length * 0.1) {
		// More than 10% are small
		issues.push(
			`${smallChunks.length} chunks are very small (<100 characters)`
		);
	...
	if (largeChunks.length > 0) {
		issues.push(
			`${largeChunks.length} chunks are very large (>2000 characters)`
		);
