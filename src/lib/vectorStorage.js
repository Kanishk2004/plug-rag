...
export async function processFileToVectors(userId, botId, fileId) {
	try {
		...
		if (chunks.length === 0) {
			...
			console.log(
				'No chunks found, attempting to create them from file content...'
			);

			if (!file.extractedText || file.extractedText.length === 0) {
				throw new Error(
					'No chunks found and no extracted text available for file'
				);
			}
			...
			console.log(
				`✅ Generated and saved ${chunkDocuments.length} chunks for file`
			);
...
async function processChunksToVectors(chunks, userId, botId, file) {
	try {
		...
		console.log(
			`Generating embeddings for ${chunks.length} chunks from file ${file.originalName}`
		);
		...
		for (const chunk of chunks) {
			try {
				...
				console.error(
					`Failed to generate embedding for chunk ${chunk._id}:`,
					embeddingError
				);
				throw new Error(
					`Embedding generation failed: ${embeddingError.message}`
				);
			}
		}
		...
		console.log(
			'⚠️ Vector storage is temporarily disabled - skipping Qdrant storage'
		);
		...
		return {
			...
			message:
				'Embeddings generated successfully. Vector storage temporarily disabled.',
		};
	} catch (error) {
...
export async function searchSimilarContent(userId, botId, query, options = {}) {
	...
	throw new Error(
		'Vector search temporarily disabled. Will be rebuilt from scratch.'
	);
}
