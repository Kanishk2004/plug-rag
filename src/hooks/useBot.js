...
export function useBot(botId) {
	...
	const fetchBot = useCallback(async (signal) => {
		if (!botId) return;
		...
		try {
			...
			const response = await fetch(`/api/bots/${botId}`, { signal });

			if (!response.ok) {
				throw new Error(
					`Failed to fetch bot: ${response.status} ${response.statusText}`
				);
			}
	...
	const updateBot = useCallback(
		async (updates) => {
			...
			try {
				...
				if (!response.ok) {
					throw new Error(
						data.error || `Failed to update bot: ${response.status}`
					);
				}
	...
	const deleteBot = useCallback(async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					data.error || `Failed to delete bot: ${response.status}`
				);
			}
	...
	useEffect(() => {
		...
		fetchBot(abortController.signal);
